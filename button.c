...
int probeButtonPath(char *newPath){
	...
	number2 = tmpStr[strlen(tmpStr)-3] - '0';
	...
	fclose(fp);
	if (returnValue == 1){
		sprintf (newPath,"%s%d",INPUT_DEVICE_LIST,number2);
	}
...
void buttonThFunc(void)
{
	...
	msgsnd(msgBUTTON,&A, sizeof(unsigned short) * 2 + sizeof(int) , 0);
	}
...
int buttonStart(void){
	msgBUTTON = msgget (MESSAGE_BU, IPC_CREAT|0666);
	if (msgBUTTON == -1){	
		printf ("Cannot get msgQueueID, Return!\r\n");
		return -1;
	}
BUTTON_MSG_T trashCan;
while (msgrcv (msgBUTTON, &trashCan, sizeof(BUTTON_MSG_T)-sizeof(long int), 0, IPC_NOWAIT) >=0)
	{
